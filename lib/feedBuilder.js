...
function xmlPriceFormatter(price) {
  return price && !isNaN(price.price) && `${price.price.toFixed(2)} ${price.currency && price.currency.toUpperCase()}`;
}
...
function xmlFixedNumberFormatBuilder(precision) {
  return (number) => {
    return !isNaN(number) && number.toFixed(precision);
  }
}
...
const xmlPropertiesMap = {
  ...
  additionalImageLink: { xmlName: 'g:additional_image_link' },
  ...
  productType: { xmlName: 'g:product_type' },
  ...
  promotionId: { xmlName: 'g:promotion_id' },
  // TODO: error in spec
  includedDestination: { xmlName: 'g:included_destination' },
  excludedDestination: { xmlName: 'g:excluded_destination' },
  shipping: {
    xmlName: 'g:shipping',
    xmlFormatter: xmlObjectFormatter,
    allowRepeat: true,
...
function xmlSingleItemProcessor(product, map, root) {
  if (map.xmlFormatter) {
    const result = map.xmlFormatter(product, map.items, root);
    if (map.xmlName) {
      return result;
    }
  } else {
